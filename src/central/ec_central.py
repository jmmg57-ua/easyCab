...
class ECCentral:
    ...
    def finalize_trip_if_needed(self, taxi):
        """Notifica al cliente si el taxi ha finalizado el viaje y procesa solicitudes pendientes."""
        try:
            if taxi.status == "END":
                self.notify_customer(taxi)
                taxi.status = "FREE"
                taxi.customer_asigned = "x"
                self.save_taxis(self.taxis)
                logger.info(f"Taxi {taxi.id} is now FREE and ready for new requests.")
                # Llamar a la reasignación de solicitudes pendientes
                self.check_pending_requests_direct(taxi)
        except Exception as e:
            logger.error(f"Error in finalize_trip_if_needed: {e}")
    ...
    def check_pending_requests_direct(self, taxi):
        """Procesa una solicitud pendiente directamente al liberar un taxi."""
        try:
            if self.pending_requests:
                ...
                customer_location = pending_request['customer_location']
                logger.info(f"Assigning taxi {taxi.id} to pending request for customer {customer_id}.")
                self.assign_taxi_to_customer(taxi, customer_id, customer_location, destination)
            else:
                logger.info(f"No pending requests to assign for taxi {taxi.id}.")
        except Exception as e:
            logger.error(f"Error in check_pending_requests_direct: {e}")

    def process_customer_request(self, request):
        """Procesa una solicitud de cliente y la asigna si hay taxis disponibles."""
        try:
            ...
            if available_taxi and available_taxi.status == "FREE":
                ...
                logger.warning(f"No available taxis for customer {customer_id}. Adding request to queue.")
                self.pending_requests.append(request)
                return False
        except Exception as e:
            logger.error(f"Error in process_customer_request: {e}")
    ...
    def run(self):
        ...
        pending_thread = threading.Thread(target=self.check_pending_requests_direct, daemon=True)
